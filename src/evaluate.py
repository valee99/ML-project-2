...
from src.predict import get_masks, combine_mask_patches, select_slices_for_output, combine_mask_models
import argparse
import ultralytics
import tifffile as tiff

def visualise(img_name: str, segmented: bool, path_dataset_patch_images: str, path_dataset_full_images: str, path_model_full: str, path_model_patch: str, use_patch: bool, n_rows_patch: int = 0, n_cols_patch: int = 0):
    """
    ...
    for slice_idx, slice_path in tqdm(enumerate(path_slices_full_image_sorted)):
        ...
        image_mask = get_masks(path_model_full, [slice_path], image_height, image_width)[0]
        ...
        if use_patch:
            patches_masks = get_masks(path_model_patch, path_patches_sorted, patch_height, patch_width)
            combined_patches_masks = combine_mask_patches(patches_masks, n_patches, n_rows_patch, n_cols_patch, (image_height, image_width))
            combined_patches_masks = binary_fill_holes(combined_patches_masks).astype(int)
    ...
    if use_patch:
        ...
        combined_slices, combined_images = select_slices_for_output(all_slices_big_model, all_images)
    ...
    def display_boxes(start_slice: int, min_contrast: int, max_contrast: int):
        ...
        if segmented:
            ...
            axes[0].imshow(img[start_slice], cmap="gray", vmin=min_contrast, vmax=max_contrast)
            axes[0].imshow(np.where(img_mask[start_slice]==0,np.nan,start_slice+1), cmap="viridis", alpha=0.5, norm=norm)
            axes[0].axis('off')
            axes[1].imshow(np.where(img_mask[start_slice]==1, img[start_slice], 255), cmap="gray", vmin=min_contrast, vmax=max_contrast)
            axes[1].axis('off')
        else:
            fig, axes = plt.subplots(1, 1, figsize=(20, 10))
            norm = Normalize(vmin=1, vmax=101)
            axes.imshow(img[start_slice], cmap="gray", vmin=min_contrast, vmax=max_contrast)
            axes.imshow(np.where(img_mask[start_slice]==0,np.nan,start_slice+1), cmap="viridis", alpha=0.5, norm=norm)
            axes.axis('off')
...
def get_mask_from_gt(labeled_folder_path: str, img_name: str) -> np.array:
    ...
    for slice_path in slices:
        small_label_path = slice_path.replace("images","small_labels").replace(".jpg",".txt")
        big_label_path = slice_path.replace("images","big_labels").replace(".jpg",".txt")
        ...
        if exists(small_label_path):
            ...
            for line in labels:
                ...
                mask_gt = cv2.drawContours(mask_gt, contours, -1, (255), thickness=cv2.FILLED)
...
def evaluate_final_output(mask_gt: np.array, mask_pred: np.array) -> dict:
    ...
    weigthed_mDice = np.sum([sizes[i]*best_dices[i] for i in range(len(sizes))]) / np.sum(sizes)
    weigthed_mDice_non_zero = np.sum([sizes_non_zero[i]*best_dices_non_zero[i] for i in range(len(best_dices_non_zero))]) / np.sum(sizes_non_zero)

    perf_dict = {"true_positive":tp,"false_positive":fp,"false_negative":fn,"recall":recall,"precision":precision,"f1-score":f1_score,"mDice":mDice,"nonzero_mDice":mDice_non_zero,"weighted_mDice":weigthed_mDice,"nonzero_weighted_mDice":weigthed_mDice_non_zero,"best_dices":best_dices,"best_dices_non_zero":best_dices_non_zero,"sizes":sizes, "sizes_nonzero":sizes_non_zero}

    return perf_dict

def main(path_labeled_folder: str, path_model_full: str, path_dataset_full: str, path_model_patch: str, path_dataset_patch: str, use_patch: bool, n_rows_patch: int = 0, n_cols_patch: int = 0):
    ...
    """
    if use_patch:
        path_list = [path_labeled_folder,path_model_full,path_dataset_full,path_model_patch,path_dataset_patch]
    else:
    ...
    if use_patch and (n_cols_patch <= 0 or n_rows_patch <= 0):
        raise ValueError("The number of patches rows and columns can't be negative if you use patches")
    
    if use_patch:
        images_small = set([basename(file_path).split("_")[0] for file_path in glob(join(path_dataset_patch,"images","*.jpg"))])
        images_big = set([basename(file_path).split("_")[0] for file_path in glob(join(path_dataset_full,"images","*.jpg"))])
        images = list(images_small.union(images_big))
    else:
        images = list(set([basename(file_path).split("_")[0] for file_path in glob(join(path_dataset_full,"images","*.jpg"))]))
    ...
    for img_name in images:
        ...
        if use_patch:
            _, _, _, combined_slices, _ = visualise(img_name, True, path_dataset_patch, path_dataset_full, path_model_full, path_model_patch, True, n_rows_patch, n_cols_patch)
        else:
            # Get the mask from the combined model
            _, _, _, combined_slices, _ = visualise(img_name, True, None, path_dataset_full, path_model_full, None, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_labeled_folder", type=str
    )
    parser.add_argument(
        "--path_model_full", type=str
    )
    ...
    main(args.path_labeled_folder, args.path_model_full, args.path_dataset_full, args.path_model_patch, args.path_dataset_patch, args.use_patch, args.n_rows_patch, args.n_cols_patch)